# start_server.py — reusable across all phases
#
# KID-FRIENDLY VERSION:
#     This is the LEMONADE STAND that sets up the table and shouts "OPEN!"
#     so customers (Teams, the Emulator) can walk up and order. The brain
#     (agent_app) makes the lemonade; this file just runs the stand.

# Allow new-style type hints on older Python versions.
...
# Imported only as a TYPE so editors can autocomplete.
...
def start_server(agent_app: AgentApplication, adapter: CloudAdapter | None = None) -> None:
    # If the caller didn't pass a phone, give them a default one.
    if adapter is None:
        adapter = CloudAdapter()

    # This runs for EVERY incoming POST to /api/messages.
    ...
    # BLOCKS forever, taking orders until you Ctrl+C.
