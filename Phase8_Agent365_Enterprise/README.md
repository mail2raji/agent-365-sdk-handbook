...
These are also in [`../requirements.txt`](../requirements.txt) under the **Phase 8** section (commented out by default).
...
[`code/governed_helpdesk_agent/app.py`](code/governed_helpdesk_agent/app.py) shows the full wiring. Imports of `microsoft_agents_a365` are guarded with `try/except`, so the file still loads if you haven't installed the pre-release packages yet — it will degrade to the Phase 6 behavior.
